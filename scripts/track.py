...
def day_code_files(block: dict, day: dict) -> list[Path]:
    """
    Us din ke practice files dhoondo: learning/<folder>/<DAYID>-*/**.py
    Auto-discovery hai — jo file bane, README me khud aa jaye.
    """
    folder = block.get("folder")
    if not folder:
        return []
    base = ROOT / "learning" / folder
    if not base.exists():
        return []
    files: list[Path] = []
    for day_dir in sorted(base.glob(f"{day['id']}-*")):
        if day_dir.is_dir():
            files.extend(sorted(p for p in day_dir.rglob("*.py") if p.is_file()))
    return files


def gen_phase_readme(block: dict, progress: dict) -> str:
    bs = block_stats(block, progress)
    L = []
    A = L.append
    A(f"# {block['title']}")
    ...
    A(
        f"`{block['days'][0]['id']}–{block['days'][-1]['id']}` · "
        f"roadmap ref `{block['roadmap_ref']}` · "
        f"**{bs['done']}/{bs['total']} days** `{bar(bs['percent'], 16)}` {bs['percent']}%"
    )
    A("")
    A(
        "Poore safar ka board: [README](../../README.md) · "
        "Live dashboard: [dashboard](https://bilalahmad72.github.io/ai-engineering-roadmap/)"
    )
    A("")
    A("---")
    A("")
    A("## Is phase ke din")
    A("")
    A("| Day | Topic | Status | Tasks | Notes |")
    A("|---|---|---|---|---|")
    for day in block["days"]:
        st = progress["days"][day["id"]]
        tdone = len([i for i in st["tasks_done"] if i < len(day["tasks"])])
        ...
        A(
            f"| [`{day['id']}`](#{day['id'].lower()}) | {day['title']} | "
            f"{STATUS_EMOJI[st['status']]} | {tdone}/{len(day['tasks'])} | {notes} |"
        )
    A("")
    A("---")
    A("")
    for day in block["days"]:
        ...
        A(
            f"{STATUS_EMOJI[st['status']]} **{STATUS_LABEL[st['status']]}** · "
            f"{day.get('hours', 2)}h"
            + (f" · started {st['started']}" if st["started"] else "")
            + (f" · completed {st['completed']}" if st["completed"] else "")
        )
        A("")
        A("**Objectives**")
        A("")
        ...
        A("")
        for i, t in enumerate(day["tasks"]):
            A(f"- [{'x' if i in st['tasks_done'] else ' '}] {t}")
        A("")
        A(f"**Deliverable:** {day['deliverable']}")
        A("")
        files = day_code_files(block, day)
        if files:
            A("**Practice code**")
            A("")
            for f in files:
        ...
        if st["notes"]:
            ...
            A("")
        if st["log"]:
            A("<details><summary>Activity log</summary>")
            A("")
            for entry in st["log"]:
                A(f"- `{entry.get('at', '')}` — {entry.get('text', '')}")
            A("")
            A("</details>")
            A("")
        A("---")
        A("")
    A("<sub>Generated by `scripts/track.py` — hath se edit mat karo.</sub>")
    return "\n".join(L) + "\n"
...
def sync(blocks: list[dict], progress: dict) -> dict:
    ...
    # Har phase ka apna README — GitHub par folder kholte hi poora phase nazar aaye.
    for block in blocks:
        folder = block.get("folder")
        if not folder:
            continue
        phase_dir = ROOT / "learning" / folder
        phase_dir.mkdir(parents=True, exist_ok=True)
        (phase_dir / "README.md").write_text(
            gen_phase_readme(block, progress), encoding="utf-8"
        )
    return stats
